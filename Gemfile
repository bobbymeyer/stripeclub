...
# Pandatone is not published to RubyGems; the engine takes it from the main
# branch of its repository rather than from a tag. A tag cannot exist until
# the change that needs it has merged, and this Gemfile is only what the
# dummy under test/ runs on — what a host resolves is the gemspec, which
# asks for a version. Running the suite against Pandatone's tip is the
